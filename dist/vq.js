...
function createDOMPurify() {
  ...
  const _checkSvgNamespace = function _checkSvgNamespace2(tagName, parent2, parentTagName) {
  ...
  const _checkMathMlNamespace = function _checkMathMlNamespace2(tagName, parent2, parentTagName) {
    if (parent2.namespaceURI === HTML_NAMESPACE) {
      return tagName === "math";
    }
  ...
  const _checkHtmlNamespace = function _checkHtmlNamespace2(tagName, parent2, parentTagName) {
  ...
  const _checkValidNamespace = function _checkValidNamespace2(element) {
    ...
    if (element.namespaceURI === SVG_NAMESPACE) {
      return _checkSvgNamespace(tagName, parent2, parentTagName);
    }
    if (element.namespaceURI === MATHML_NAMESPACE) {
      return _checkMathMlNamespace(tagName, parent2, parentTagName);
    }
    if (element.namespaceURI === HTML_NAMESPACE) {
      return _checkHtmlNamespace(tagName, parent2, parentTagName);
    }
...
var $html = function(value = null) {
  ...
  this.innerHTML = purify.sanitize(value, { ADD_ATTR: ["target"] });
  return this;
};
...
function select(query, scope = document) {
  ...
  if (el?.tagName === "TEMPLATE") {
    const clone2 = el.content.cloneNode(true);
    return extend(clone2);
  }
  return extend(el);
}
...
function html2(strings, ...values) {
  ...
  template.innerHTML = purify.sanitize(string, { ADD_ATTR: ["target"] });
...
async function whenDefined(components = {}) {
  const entries2 = Object.entries(components);
  for (const [name, element] of entries2) {
    if (customElements.get(name))
      continue;
    else
      customElements.define(name, element);
  }
  await Promise.all(entries2.map(([name]) => customElements.whenDefined(name)));
}
